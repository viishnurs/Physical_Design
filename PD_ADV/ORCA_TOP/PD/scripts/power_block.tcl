...
########################################## Macro rings #################################
create_pg_ring_pattern BLOCK_RING_PATTERN -horizontal_layer M5 -vertical_layer M6 -horizontal_width 0.52 -vertical_width 0.52

set_pg_strategy BLOCK_RING_VDD_STRATEGY -pattern {{name: BLOCK_RING_PATTERN} {nets: {VDD VSS}} {offset: {0.5 0.5}}} -blocks $hm(top)

set_pg_strategy BLOCK_RING_VDDH_STRATEGY -pattern {{name: MACRO_RING_PATTERN} {nets: {VDDH VSS}} {offset: {0.5 0.5}}} -blocks $hm(risc_core)

set_pg_strategy_via_rule S_ring_vias -via_rule {\
			{{{strategies: {BLOCK_RING_VDD_STRATEGY BLOCK_RING_VDDH_STRATEGY}} {layers: {M5}}} {existing: {strap}} {via_master: {default}}} \
			{{{strategies: {BLOCK_RING_VDD_STRATEGY BLOCK_RING_VDDH_STRATEGY}} {layers: {M6}}} {existing: {strap}} {via_master: {default}}} \
	 }
compile_pg -strategies {BLOCK_RING_VDD_STRATEGY MACRO_RING_VDDH_STRATEGY} -via_rule S_ring_vias

########### Macro pin connection ####################################
create_pg_block_conn_pattern P_HM_pin -pin_conn_type scattered_pin  -layers {M5 M6}
set_pg_strategy S_HM_top_pins -blocks $hm(top) -pattern { {pattern: P_HM_pin} {nets: {VSS VDD}} }
set_pg_strategy S_HM_risc_pins -blocks $hm(risc_core) -pattern { {pattern: P_HM_pin} {nets: {VSS VDDH}} }
